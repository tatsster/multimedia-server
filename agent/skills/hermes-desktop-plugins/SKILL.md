...
system (`~/.agent/hermes/plugins/`) is otherwise documented separately.
...
The ONLY import surface is `@agent/hermes/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.agent/hermes/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@agent/hermes/plugin-sdk`, `react`, and
