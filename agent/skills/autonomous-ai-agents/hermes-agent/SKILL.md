...
## Umbrella scope: Hermes development and operations

This skill is the class-level umbrella for working on Hermes Agent itself: configuration, extension, debugging, skill authoring, TUI slash commands, container supervision, prompts, gateways, profiles, tools, memory, and in-repo development practices. Prefer this umbrella over narrow Hermes debugging/session-artifact skills when the task is about the Hermes codebase.

### Webhook/event subscriptions

For Hermes webhook subscriptions and event-driven agent runs, manage them as gateway/platform operations here: enable the webhook platform, create/list/remove/test `hermes webhook` subscriptions, handle HMAC secrets, deliver-only routes, and gateway health/log troubleshooting. The archived `webhook-subscriptions` package contains the full command examples and service-specific patterns.

### MCP client integration

For Hermes native MCP client work, manage `mcp_servers` in config, stdio/HTTP transports, startup discovery, tool naming, sampling, reconnect behavior, and server troubleshooting. The archived `native-mcp` package contains the full configuration reference and examples.

### Skill authoring

When authoring or modifying skills in the Hermes repo, preserve frontmatter, clear trigger conditions, exact commands, support files under `references/`, `templates/`, `scripts/`, and validation instructions. The archived `hermes-agent-skill-authoring` package contains the former narrow checklist.

### TUI slash command debugging

For slash commands, inspect both Python/backend command registration and the Ink/React TUI path. Reproduce with the TUI where possible and run the relevant test suite. The archived `debugging-hermes-tui-commands` package contains the former focused recipe.

### s6 container supervision

For Docker/s6-overlay work, understand the supervision tree and Architecture B main-program pattern before changing services. The archived `hermes-s6-container-supervision` package contains the former focused container-supervision notes.
...
/reload-skills       Re-scan ~/.agent/hermes/skills/ for added/removed skills
...
```
~/.agent/hermes/config.yaml       Main configuration
~/.agent/hermes/.env              API keys and secrets
$HERMES_HOME/skills/        Installed skills
~/.agent/hermes/sessions/         Gateway routing index, request dumps, *.jsonl transcripts (and optional per-session JSON snapshots when sessions.write_json_snapshots: true)
~/.agent/hermes/state.db          Canonical session store (SQLite + FTS5)
~/.agent/hermes/logs/             Gateway and error logs
~/.agent/hermes/auth.json         OAuth tokens and credential pools
~/.agent/hermes/hermes-agent/     Source code (if git-installed)
```

Profiles use `~/.agent/hermes/profiles/<name>/` with the same layout.
...
| `stt` | `enabled`, `provider` (local/groq/openservices/ai/mistral) |
| `tts` | `provider` (edge/elevenlabs/openservices/ai/minimax/mistral/neutts) |
...
Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.agent/hermes/shell-hooks-allowlist.json` — prompted interactively the first time a hook wants to run.
...
- **Telemetry:** sidecar at `~/.agent/hermes/skills/.usage.json` holds
...
Check logs first:
```bash
grep -i "failed to send\|error" ~/.agent/hermes/logs/gateway.log | tail -20
```
...
| Gateway logs | `~/.agent/hermes/logs/gateway.log` |
| Session files | `hermes sessions browse` (reads state.db) |
| Source code | `~/.agent/hermes/hermes-agent/` |
...
Config: `~/.agent/hermes/config.yaml` (settings), `~/.agent/hermes/.env` (API keys).
...
- Tests auto-redirect `HERMES_HOME` to temp dirs — never touch real `~/.agent/hermes/`
