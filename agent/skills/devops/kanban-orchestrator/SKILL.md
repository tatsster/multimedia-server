...
## Umbrella scope: Kanban roles

This skill is the class-level umbrella for Hermes Kanban work. It covers both orchestrator and worker behavior: decomposition, routing, card lifecycle, worker handoffs, reconciliation, and the anti-temptation rule that the orchestrator should not do implementation work itself.

### Worker role summary

Workers should keep their card state current, ask for clarification only through the orchestrator channel when needed, produce verifiable handoffs, and avoid scope creep. The archived `kanban-worker` skill contained deeper worker-specific pitfalls and examples; restore/re-home that package if detailed worker edge cases are needed.
