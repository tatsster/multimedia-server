...
2. The GitHub repository config/docs, currently `tatsster/multimedia-server` under the `homepage/` directory.
...
2. **Update the GitHub repository**
   ...
   - Edit relevant files under `homepage/`, commonly:
     - `homepage/services.yaml`
     - `homepage/widgets.yaml` if applicable
     - `homepage/.env.example` for new/removed API tokens, URLs, usernames, or passwords
     - `homepage/Readme.md` for docs/service lists/env tables
...
3. **Validate repository config**
   - Run YAML validation for Homepage YAML files, for example:
     ...
     python3 - <<'PY'
     import glob, yaml
     for f in glob.glob('homepage/*.yaml'):
         with open(f) as fh:
             yaml.safe_load(fh)
         print('OK', f)
     PY
...
5. **Update the live Homepage CT/LXC config**
   ...
   - Edit the live Homepage config to match the repository changes.
