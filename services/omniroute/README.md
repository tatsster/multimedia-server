...
OmniRoute is the OpenAI-compatible AI gateway/router used by Hermes and other local agents.
...
Node: v24.15.0
npm: 11.12.1
Binary: /usr/bin/omniroute -> /usr/lib/node_modules/services/omniroute/bin/omniroute.mjs
Data dir: /root/.omniroute
SQLite DB: /root/.services/omniroute/storage.sqlite
Backups: /root/.services/omniroute/db_backups
Call logs: /root/.services/omniroute/call_logs
...
```text
services/omniroute/systemd/omniroute.service.example
```

Copy it to:

```bash
cp /root/repos/multimedia-server/services/omniroute/systemd/omniroute.service.example /etc/systemd/system/omniroute.service
...
```text
/root/.services/omniroute/omniroute.env
```
...
| Hermes/agent API key | Clients calling `http://192.168.1.109:20128/v1` | Create in OmniRoute dashboard API keys section after login. Store in Hermes or agent env/config, never in this repo. |
...
bash automation/pve/create-omniroute-lxc.sh
```
...
cp /root/repos/multimedia-server/services/omniroute/config/omniroute.env.example /root/.services/omniroute/omniroute.env
chmod 0600 /root/.services/omniroute/omniroute.env
nano /root/.services/omniroute/omniroute.env
```
...
Add provider connections and create client API keys for Hermes or other local agents. Store those keys only in private env/config on each client LXC.
...
```text
/usr/bin/omniroute-reset-password -> /usr/lib/node_modules/services/omniroute/bin/reset-password.mjs
```

The upstream helper expects `DATA_DIR/settings.db`, but this live install uses:

```text
/root/.services/omniroute/storage.sqlite
```
...
const db = new Database('/root/.services/omniroute/storage.sqlite');
...
```bash
sqlite3 /root/.services/omniroute/storage.sqlite   "select namespace,key,case when key='password' then '[REDACTED]' else value end from key_value where namespace='settings' order by key;"
```
...
Compose file: /root/services/omniroute/docker-compose.yml
Data dir: /root/services/omniroute/data
Generated secrets: /root/services/omniroute/data/server.env
```

Do not manually add `JWT_SECRET` to `/root/services/omniroute/.env`. OmniRoute generates and reads it from `data/server.env`.
...
```text
services/omniroute/metrics-infrastructure/proxy/
```
...
The proxy runs beside OmniRoute on CT `107`, reads `/root/services/omniroute/data/server.env` read-only, creates a short-lived dashboard JWT, calls OmniRoute with `Cookie: auth_token=...`, and exposes a simple token-protected endpoint:
...
```text
services/omniroute/metrics-infrastructure/proxy/README.md
```
