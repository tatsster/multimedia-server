...
It is not Glance-specific. Glance, Homepage, Grafana JSON/API panels, scripts, or any other trusted LAN dashboard can call it.
...
1. Reads OmniRoute's local `JWT_SECRET` from `/root/omniroute/data/server.env`.
...
OmniRoute data/secrets file on CT107: /root/omniroute/data/server.env
Proxy env file on CT107: /root/omniroute/metrics-proxy.env
...
```text
omniroute/metrics-proxy/server.js
omniroute/metrics-proxy/package.json
omniroute/metrics-proxy/Dockerfile
omniroute/metrics-proxy/docker-compose.example.yml
omniroute/metrics-proxy/.env.example
```
...
```bash
TOKEN="$(grep '^OMNIROUTE_DASHBOARD_TOKEN=' /root/omniroute/metrics-proxy.env | cut -d= -f2-)"
...
Backward-compatible env names still work for existing Glance installs:

```text
OMNIROUTE_GLANCE_TOKEN
...
cd /root/repos/multimedia-server/omniroute/metrics-proxy

docker build -t omniroute-metrics-proxy:local .

mkdir -p /root/omniroute
if [ ! -f /root/omniroute/metrics-proxy.env ]; then
  cp .env.example /root/omniroute/metrics-proxy.env
  TOKEN="$(openssl rand -hex 32)"
  sed -i "s|^OMNIROUTE_DASHBOARD_TOKEN=.*|OMNIROUTE_DASHBOARD_TOKEN=${TOKEN}|" /root/omniroute/metrics-proxy.env
fi

chmod 600 /root/omniroute/metrics-proxy.env
sed -i "s|^OMNIROUTE_URL=.*|OMNIROUTE_URL=http://172.17.0.1:20128|" /root/omniroute/metrics-proxy.env
sed -i "s|^PORT=.*|PORT=20129|" /root/omniroute/metrics-proxy.env

docker rm -f omniroute-glance-proxy omniroute-metrics-proxy 2>/dev/null || true

docker run -d \
  --name omniroute-metrics-proxy \
  --restart unless-stopped \
  --env-file /root/omniroute/metrics-proxy.env \
  -p 20129:20129 \
...
```bash
cp /root/repos/multimedia-server/omniroute/metrics-proxy/.env.example /root/omniroute/metrics-proxy.env
chmod 600 /root/omniroute/metrics-proxy.env
```
...
Do not commit `/root/omniroute/metrics-proxy.env` or the generated token.
...
```bash
mkdir -p /root/omniroute/metrics-proxy
cp /root/repos/multimedia-server/omniroute/metrics-proxy/docker-compose.example.yml /root/omniroute/metrics-proxy/docker-compose.yml
cd /root/omniroute/metrics-proxy
...
OMNIROUTE_DASHBOARD_TOKEN=<same token from /root/omniroute/metrics-proxy.env>
```
...
The included Glance widget is:

```text
glance/widgets/omniroute.yml
```

It uses the generic env vars:
...
## Migration from the old `glance-proxy` name

Old names remain partially supported in code for compatibility, but new deployments should use the generic names.
...
# Reuse the existing token if the old env file exists.
mkdir -p /root/omniroute
if [ -f /root/omniroute/glance-proxy.env ] && [ ! -f /root/omniroute/metrics-proxy.env ]; then
  OLD_TOKEN="$(grep '^OMNIROUTE_GLANCE_TOKEN=' /root/omniroute/glance-proxy.env | cut -d= -f2-)"
  cp .env.example /root/omniroute/metrics-proxy.env
  sed -i "s|^OMNIROUTE_DASHBOARD_TOKEN=.*|OMNIROUTE_DASHBOARD_TOKEN=${OLD_TOKEN}|" /root/omniroute/metrics-proxy.env
fi
...
Update dashboard env vars from:

```text
OMNIROUTE_GLANCE_PROXY_URL=...
OMNIROUTE_GLANCE_TOKEN=...
```
...
```text
OMNIROUTE_METRICS_PROXY_URL=...
OMNIROUTE_DASHBOARD_TOKEN=...
```
