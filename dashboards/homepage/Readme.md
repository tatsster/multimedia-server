...
| Config path | `/opt/dashboards/homepage/config` |
...
```text
dashboards/homepage/settings.yaml
dashboards/homepage/services.yaml
dashboards/homepage/widgets.yaml
dashboards/homepage/bookmarks.yaml
dashboards/homepage/docker.yaml
dashboards/homepage/kubernetes.yaml
dashboards/homepage/.env.example
```

These files are secret-safe templates for the live `/opt/dashboards/homepage/config` directory. Real tokens/passwords stay only in the live Homepage environment or local secret files.
...
```bash
mkdir -p /opt/dashboards/homepage/config
cp dashboards/homepage/settings.yaml /opt/dashboards/homepage/config/settings.yaml
cp dashboards/homepage/services.yaml /opt/dashboards/homepage/config/services.yaml
cp dashboards/homepage/widgets.yaml /opt/dashboards/homepage/config/widgets.yaml
cp dashboards/homepage/bookmarks.yaml /opt/dashboards/homepage/config/bookmarks.yaml
cp dashboards/homepage/docker.yaml /opt/dashboards/homepage/config/docker.yaml
cp dashboards/homepage/kubernetes.yaml /opt/dashboards/homepage/config/kubernetes.yaml
```
...
Use Beszel as the single source of truth for Proxmox host monitoring, history, and alerts. Homepage keeps Beszel as a normal service link/monitor card in `services.yaml`. OpenViking is the active knowledge/memory service:
